...
void array_from_file(WeatherTable array, const char *filepath) {
    ...
    while (!feof(file)) {
        int res = fscanf(file, " %u %u %u ", &k_year, &k_month, &k_day); /* buena practica, ya que si el arreglo no tiene 3 elementos, debe fallar*/
        if (res != 3) {
            fprintf(stderr, "Invalid array.\n");
            exit(EXIT_FAILURE);
        }
        Weather weather = weather_from_file(file);
        array[k_year - FST_YEAR][k_month - 1][k_day - FST_DAY] = weather; /* aca le resta fst year pq k_year va a vale 1980, pero el elemento no es el 1980, sino que el indice es el año 0 */
        }
    fclose(file);
}
