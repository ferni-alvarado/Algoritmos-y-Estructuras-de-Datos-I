...
static bool invrep(graph_t g) {
    bool check = g!= NULL;
    for(unsigned int i = 0; i < g->max_vertexs && check; ++i) {
        check = g->costs[g_index(i, i, g->max_vertexs)] == 0;
    }    
    return check;
}
...
graph_t graph_empty(unsigned int max_vertexs)
{
    ...
    for (unsigned int i = 0; i < max_vertexs; ++i) {
        graph->costs[g_index(i, i, max_vertexs)] = 0;
    }    
...
void graph_add_edge(graph_t graph, vertex_t from, vertex_t to, cost_t cost)
{
    ...
    unsigned int i = g_index(from, to, graph->max_vertexs);
    graph->costs[i] = cost;
    assert(invrep(graph));
}
